...
author: "Chief Science Officer TONGFEI ZHOU; ID: 1004738448"
output:
...
header-includes:    
  ...
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[LO, LE]{TONGFEI ZHOU, 1004738448}
...
The name is Maria Devora.

```{r}
Communications_Officer <- crew_data %>%
  filter(position == "Communications Officer")
```

3. How many crewmembers are in this dataset? 

By calculating different names, I get 251 crewmembers.

```{r}
crew_data %>%
  filter(is.na(name) | is.na(crew_id)) # no NA
crewmembers <- length(unique(crew_data$name))
```
...
```{r}
unique(crew_data$position)
```

As we can see from above, the position title "Enigneering Technician" has misspelled the word "Engineering", and so we fix it as follows and perform the rest operations.

```{r}
crew_data <- crew_data %>%
  mutate(position = case_when(
    position == "Enigneering Technician" ~ "Engineering Technician",
    TRUE ~ position
  ))
Eng <- crew_data %>% 
  filter(sub_division == "Engineering") %>%
  filter(str_detect(position, "Engineering") | str_detect(position, "Engineer"))
Eng_sub <- crew_data %>% 
  filter(sub_division == "Engineering")
pro <- length(unique(Eng$name)) / length(unique(Eng_sub$name))
pro
```

So the proportion is 0.6206897.
...
week1 <- crew_data %>%
  filter(week == 1) %>%
  mutate(duty_shift = factor(duty_shift, levels = c("Gamma", "Delta", "Beta", "Alpha")))
table(week1$duty_shift)
```

As we can see above the factor has been reordered successfully.
...
Since the p-value is smaller than significance level of 5%, we have strong evidence to reject the hypothesis that the simpler model fits the data just as well, and so we will choose model 1b.
...
Between model 1b and model 2a, we will choose model 2a by the same reasons above, but between model 2a and model 2b, the p-value indicates that there is no difference between these two models. As model 2a contains an interaction term, I choose model 2b.
...
model_3 <- lmer(productivity ~ week + starfleet_gpa + perseverance_score + 
                  (1 + week|name) + (1 + week|full_team), 
                 data = crew_data)

lmtest::lrtest(model_2b, model_3)
```

As the p-value is greater than 0.05, we fail to reject the null hypothesis and so we will choose the simpler model, i.e, the one without random slope for week based on full_team random effect. So I will choose model 2b.
...
origin_data %>% 
  filter(nativeto == "Delta Quadrant")
#> Error in origin_data %>% filter(nativeto == "Delta Quadrant"): could not find function "%>%"
```

<sup>Created on 2021-02-22 by the [reprex package](https://reprex.tidyverse.org) (v0.3.0)</sup>
